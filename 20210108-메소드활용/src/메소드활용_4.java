 ...
 *    ===============
 *    변수 => 배열 
 *    명령문 => 메소드    ===========> 클래스 (여러개를 다시 묶는다(관련된 클래스끼리)) ===> 패키지
 *    =====
 *    연산자+제어문 
 *    ===============
 */
public class 메소드활용_4 {
	static String[] title={
            "Your Lights",
			"겨울잠",
			"우린 어쩌다 헤어진 걸까",
	...
	public static void main(String[] args) {
        ...
        //1. 전체목록 출력 => for-each(X)
		/*
		System.out.println("===== 뮤직 Top50 전체 목록 =====");
		for(int i=0;i<50;i++)
		{
			...
			System.out.println("변동폭:"+(s+" "+(modify[i]==0?"":modify[i])));
			System.out.println("=============================================================");
		}*/
		//2. 찾기 
		...
		for(int i=0;i<50;i++)
		{
			if(title[i].contains(ss))// 문자열에 포함 여부 확인 => boolean => 오라클 (like),REGEXP_LIKE(),정규식
			{
				System.out.println("순위:"+(i+1));
				System.out.println("노래명:"+title[i]);
				System.out.println("가수명:"+singer[i]);
